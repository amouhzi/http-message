...
`HttpMessage\Request` implements [`Psr\Http\Message\RequestInterface`](https://github.com/php-fig/http-message/blob/master/src/RequestInterface.php),
...
For server-side applications, `HttpMessage\ServerRequest` implements
...
`HttpMessage\Response` provides an implementation of
...
to use for the response, typically HTML or data to serialize as JSON.  `HttpMessage\Response\StringResponse`
...
`HttpMessage\Uri` is an implementation of
...
`HttpMessage\Stream` is an implementation of
...
`HttpMessage\UploadedFile` is an implementation of
...
`HttpMessage\Server` represents a server capable of executing a callback. It has four methods:
