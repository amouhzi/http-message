...
`HttpMessage\Response\HtmlResponse` allows specifying HTML as a payload, and sets the
...
`HttpMessage\Response\JsonResponse` accepts a data structure to convert to JSON, and sets
...
`HttpMessage\Response\EmptyResponse` is a `HttpMessage\Response` extension that, by default,
...
`HttpMessage\Response\RedirectResponse` is a `HttpMessage\Response` extension for producing
